    ...
    var body: some View {
        NavigationStack {
            VStack(alignment: .leading, spacing: 0) {
                ...
                .padding()
                // Apple native forms use dynamic tint or system backgrounds for disabled state
                .background(isEmailValid && !isLoading ? Color.accentColor : Color(UIColor.systemGray5))
                .foregroundColor(isEmailValid && !isLoading ? .white : Color(UIColor.systemGray2))
                .cornerRadius(14)
        ...
        Task {
            do {
                try await supabase.auth.signInWithOTP(
                    email: email,
                    redirectTo: URL(string: "inventory-inspect://login-callback")
                )
                // Trigger navigation
