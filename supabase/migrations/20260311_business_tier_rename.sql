-- Rename the top tier from enterprise to business and keep server-side gating aligned.
-- Apply to Supabase `test` first, then to `main`.

alter table public.profiles
add column if not exists property_limit_override integer,
add column if not exists team_limit_override integer,
add column if not exists photo_limit_override integer;
...
create or replace function public.get_owner_photo_usage(
